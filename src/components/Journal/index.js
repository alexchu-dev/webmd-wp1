...
export default function LatestJournal() {
	...
	return (
		...
		>
			<h2 className="text-xl font-bold mb-2">LATEST JOURNALS</h2>
			<div className="journal-entries grid grid-cols-1 md:grid-cols-3 gap-8">
				{blog?.map(({ slug, title, image, date, excerpt, user_id }) => (
					...
					>
						<div className="relative w-full h-[300px]">
							<Link href={`journals/${user_id}/${slug}`}>
								<Image
									src={image.url}
									alt={image.alt}
									title={image.alt}
